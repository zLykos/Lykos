...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	function _processBitmap($bitmap)
	{
		...
		if ($identity['ident'] != "BM") {
			throw new PHPExcel_Writer_Exception("$bitmap doesn't appear to be a valid bitmap images.\n");
		}
		...
		$data   = substr($data, 8);
		if ($width > 0xFFFF) {
			throw new PHPExcel_Writer_Exception("$bitmap: largest images width supported is 65k.\n");
		}
		if ($height > 0xFFFF) {
			throw new PHPExcel_Writer_Exception("$bitmap: largest images height supported is 65k.\n");
		}
		...
		if ($planes_and_bitcount[1] != 1) {
			throw new PHPExcel_Writer_Exception("$bitmap: only 1 plane supported in bitmap images.\n");
		}
		...
		if ($compression['comp'] != 0) {
			throw new PHPExcel_Writer_Exception("$bitmap: compression not supported in bitmap images.\n");
		}
