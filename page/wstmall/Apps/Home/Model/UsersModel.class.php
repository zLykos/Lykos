...
class UsersModel extends BaseModel {
	...
	public function checkPaypwd(){
		...
		if(empty($payPwd)){
			...
			if($user['payPwd']==""){
				$rd['msg'] = '您还未设置支付密码，请前往用户中心设置！';
			}else if($user['payPwd']!=md5(I("payPwd").$user['loginSecret'])){
				$rd['msg'] = '支付密码不正确';
			}else{
				$rd["status"] = 1;
			}
	 ...
	 * 检测帐号是否有效
	 */
	public function checkAccValid(){
		$userId = (int)session('WST_USER.userId');
		$sql = "select userStatus from __PREFIX__users where userFlag=1 and userId=".$userId;
		$user = $this->queryRow($sql);
